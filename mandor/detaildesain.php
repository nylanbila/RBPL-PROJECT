...
if (!empty($id_desain)) {
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        // Query untuk mengambil nama desainer
        if (!empty($id_desainer)) {
            $sql_desainer = "SELECT username FROM desainer WHERE id_desainer = '$id_desainer'";
            $result_desainer = mysqli_query($koneksi, $sql_desainer);
            if ($result_desainer && mysqli_num_rows($result_desainer) > 0) {
                $row_desainer = mysqli_fetch_assoc($result_desainer);
                $nama_desainer = $row_desainer['username'];
            }
        }
        $revision_data = [];
        if (!empty($id_desain)) {
            $revision_query = "SELECT file_revisi 
                            FROM revisi_desain 
                            WHERE id_desain = '$id_desain' 
                            AND file_revisi IS NOT NULL 
                            AND file_revisi != ''
                            ORDER BY id_revisi DESC
                            LIMIT 1";
            $revision_result = mysqli_query($koneksi, $revision_query);
            if ($revision_result && mysqli_num_rows($revision_result) > 0) {
                $revision_row = mysqli_fetch_assoc($revision_result);
                $revision_data = $revision_row['file_revisi'];
            }
        }
...
<div class="flex h-screen overflow-hidden">
        ...
        <div>
            ...
            <nav class="px-6 space-y-2">
                ...
                <div class="cursor-pointer" onclick="toggleDropdown('desain-dropdown')">
                    ...
                    <div id="desain-dropdown" class="dropdown-content active pl-10 mt-2 space-y-1">
    ...
    <div class="flex-1 flex flex-col">
        ...
        <div class="flex-1 overflow-y-auto p-10">
            <!-- Page Title -->
            <div class="mb-8">
                <h2 class="text-4xl font-medium text-zinc-800 mb-2">Detail Desain</h2>
                <?php if (!empty($id_desain) && !empty($nama_desain)): ?>
                    <p class="text-neutral-500 text-xl">
                        Berikut detail desain <?php echo htmlspecialchars($nama_desain); ?>
                    </p>
                <?php endif; ?>
            </div>
                ...
                <div class="bg-white rounded-lg shadow-lg p-8">
                    ...
                    <!-- Image Section -->
                        <div class="space-y-4">
                            ...
                            $imagePath = "../desainer/" . htmlspecialchars($gambar_desain);
                            
                            // Cek apakah ada revisi terbaru dengan file_revisi untuk desain ini
                            if (!empty($revision_data)) {
                                $imagePath = "../desainer/" . htmlspecialchars($revision_data);
                            }
                            ?>
                            <img class="w-full h-[400px] lg:h-[500px] rounded-xl border border-gray-300 object-cover" 
                                src="<?php echo $imagePath; ?>" 
                                alt="<?php echo htmlspecialchars($nama_desain); ?>" />
                        </div>

                        <!-- Details Section -->
                        <div class="space-y-4">
                            <h3 class="text-3xl font-medium text-zinc-800 mb-6">
                                <?php echo htmlspecialchars($nama_desain); ?>
                            </h3>
                            
                            <div class="space-y-3 text-xl text-black">
                                <div class="flex flex-wrap">
                                    <span class="font-medium w-40 flex-shrink-0">ID Desain:</span>
                                    <span><?php echo htmlspecialchars($id_desain); ?></span>
                                </div>
                                
                                <div class="flex flex-wrap">
                                    <span class="font-medium w-40 flex-shrink-0">Nama Desainer:</span>
                                    <span><?php echo htmlspecialchars($nama_desainer); ?></span>
                                </div>
                                
                                <div class="flex flex-wrap">
                                    <span class="font-medium w-40 flex-shrink-0">Tanggal Unggah:</span>
                                    <span><?php echo htmlspecialchars($tanggal_dibuat); ?></span>
                                </div>
                                
                                <div class="flex flex-wrap">
                                    <span class="font-medium w-40 flex-shrink-0">Ukuran:</span>
                                    <span><?php echo htmlspecialchars($ukuran); ?></span>
                                </div>
                            </div>

                            <div class="mt-6 space-y-4">
                                <div>
                                    <h4 class="font-semibold text-xl text-black mb-2">Material:</h4>
                                    <p class="text-lg text-gray-700 leading-relaxed">
                                        <?php echo nl2br(htmlspecialchars($material)); ?>
                                    </p>
                                </div>
                                
                                <div>
                                    <h4 class="font-semibold text-xl text-black mb-2">Deskripsi:</h4>
                                    <p class="text-lg text-gray-700 leading-relaxed">
                                        <?php echo nl2br(htmlspecialchars($deskripsi_desain)); ?>
                                    </p>
                                </div>
                            </div>
                        </div>
            ...
            <?php else: ?>
                <div class="bg-white rounded-lg shadow-lg p-8">
                    <div class="text-center">
                        <div class="text-red-600 text-2xl font-medium mb-4">
                            <i class="fas fa-exclamation-triangle text-4xl mb-2"></i>
                            <br>
                            Desain tidak ditemukan
                        </div>
                        <p class="text-gray-600 text-lg">
                            Desain yang Anda cari tidak tersedia atau telah dihapus.
                        </p>
                        <button onclick="window.location.href='desain.php'" 
                                class="mt-4 bg-zinc-800 text-white px-6 py-2 rounded-lg hover:bg-zinc-700 transition-colors">
